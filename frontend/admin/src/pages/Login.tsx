...
export default function Login() {
  ...
  const handleDevLogin = async (values: { username: string }) => {
    try {
      const res: any = await authApi.devLogin({ username: values.username, is_admin: true });
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', background: '#f0f2f5' }}>
      <Card style={{ width: 400, textAlign: 'center' }}>
        <Title level={3}>ToolHub 管理端</Title>
        ...
        <Form onFinish={handleDevLogin} layout="inline" style={{ justifyContent: 'center' }}>
          ...
          <Form.Item style={{ marginBottom: 0 }}>
            <Button htmlType="submit">管理员登录</Button>
          </Form.Item>
        </Form>

        <Text type="secondary" style={{ display: 'block', marginTop: 12, fontSize: 12 }}>
          输入"管理员"以管理员身份登录（is_admin=true）
        </Text>
      </Card>
    </div>
  );
